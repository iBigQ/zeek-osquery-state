...
function schedule_remove(host_id: string, uid: int) {
...
event osquery::user_removed(t: time, host_id: string, uid: int, gid: int, uid_signed: int, gid_signed: int, username: string,
	...
	# Schedule remove
	schedule_remove(host_id, uid);
}

event osquery::state::users::scheduled_remove_host(host_id: string, cookie: string) {
...
event osquery::host_disconnected(host_id: string) {
	...
	if (connect_balance[host_id] == 0) {
		...
		schedule osquery::STATE_REMOVAL_DELAY { osquery::state::users::scheduled_remove_host(host_id, host_freshness[host_id]) };
		# Remove legacy
		remove_legacy(host_id);
	}
}
